 ...
 * class member is visited a {@link org.objectweb.asm.ClassVisitor}. Before the node is added to the
...
public interface ShrinkerGraph<T> {
     ...
     */
    @NonNull
    T getMemberReference(
            @NonNull String className, @NonNull String memberName, @NonNull String desc);
    ...
    /**
     * Removes all {@link DependencyType#REQUIRED_CODE_REFERENCE} and {@link
     * DependencyType#REQUIRED_CODE_REFERENCE_REFLECTION} edges that start from the given node.
     */
     ...
     */
    boolean isProgramClass(@NonNull T klass);
     ...
     * reachable nodes, keys in the map will get their counters incremented, using the {@link
     * DependencyType} from the matching map value. They can be considered as nodes with incoming
     * edges that don't have a start node.
     */
    void addRoots(@NonNull Map<T, DependencyType> symbolsToKeep, @NonNull CounterSet counterSet);
